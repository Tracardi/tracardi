...
def get_context(plugin: ActionRunner, include: List[str] = None) -> dict:
    ...
    return {
        "id": plugin.id,
        "debug": plugin.debug,
        "event": plugin.event.model_dump() if plugin.event else None,
        "session": plugin.session.model_dump() if plugin.session else None,
        "profile": plugin.profile.model_dump() if plugin.profile else None,
        "metrics": plugin.metrics,
        "memory": plugin.memory,
        "node": plugin.node.model_dump() if 'node' in include else None,
        "flow": plugin.flow.model_dump() if 'flow' in include and plugin.flow else None,
        "flow_history": plugin.flow_history if 'flow_history' in include else None,
        "execution_graph": plugin.execution_graph.model_dump() if 'execution_graph' in include else None,
        "tracker_payload": plugin.tracker_payload.model_dump() if 'tracker_payload' in include and plugin.tracker_payload else None,
        "ux": plugin.ux,
        "join": plugin.join
    }
