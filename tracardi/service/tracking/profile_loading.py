...
def _copy_tracker_payload_session_metadata(tracker_session: DefaultEntity, session: Session) -> Session:
    if tracker_session and isinstance(tracker_session, DefaultEntity) and tracker_session.metadata:
        if tracker_session.metadata.insert:
            session.metadata.time.insert = tracker_session.metadata.insert
        if tracker_session.metadata.update:
            session.metadata.time.update = tracker_session.metadata.update
        if tracker_session.metadata.create:
            session.metadata.time.create = tracker_session.metadata.create
    return session
...
def _resolve_conflicts(tracker_session: Entity, requested_profile_id, loaded_session_profile_id, profile: FlatProfile,
    ...
    if not no_profiles_conflict:
        ...
        _copy_tracker_payload_session_metadata(tracker_session, session)

        # Update tracker payload
        tracker_session.id = session.id
    ...
    return session, tracker_session


def _load_default_profile(tracker_profile, session, static: bool) -> Tuple[FlatProfile, Session, PrimaryEntity]:
    # Create new profile
    ...
    return flat_profile, session, tracker_profile


async def _load_profile_by_session_profile_id(tracker_profile, session: Session, static: bool) -> Tuple[
    FlatProfile, Session, PrimaryEntity]:
    # Check if the profile.id from session is not empty

    if not session.profile or not session.profile.id:
        return _load_default_profile(tracker_profile, session, static)
...
async def _load_profile_by_payload_profile_id(tracker_profile, tracker_session, session: Session, static: bool) -> \
Tuple[Tuple[FlatProfile, Session, PrimaryEntity], Entity]:
    ...
    if flat_profile is not None:
        ...
        session, tracker_session = _resolve_conflicts(tracker_session, requested_profile_id, loaded_session_profile_id,
                                                      flat_profile, session)

        return (flat_profile, session, tracker_profile), tracker_session

    # Profile missing in db
    conflicting_profiles = session.profile.id != get_entity_id(tracker_profile)
    if conflicting_profiles:  # if there is different profile in session lets try to load this profile
        return await _load_profile_by_session_profile_id(tracker_profile, session, static), tracker_session
    else:
        return _load_default_profile(tracker_profile, session, static), tracker_session


async def _get_profile(tracker_profile: Optional[PrimaryEntity], tracker_session: Optional[DefaultEntity],
                       session: Session, static: bool) -> Tuple[FlatProfile, Session, PrimaryEntity, DefaultEntity]:
    # Check consistency
    ...
    if _has_tracker_payload_profile_id(tracker_profile):
        ...
        (flat_profile, session, tracker_profile), tracker_session = await _load_profile_by_payload_profile_id(
            tracker_profile,
            tracker_session,
            session,
            static)

    elif _has_profile_id_in_session(session):

        # Fallback to loading from session with regular fallback

        flat_profile, session, tracker_profile = await _load_profile_by_session_profile_id(
            tracker_profile,
            session,
            static)
        ...
        flat_profile, session, tracker_profile = _load_default_profile(
            tracker_profile,
            session,
            static)
...
async def get_profile_and_session(
...
) -> Tuple[Optional[FlatProfile], Session, PrimaryEntity, DefaultEntity]:
    """
    ...
    if profile_less is True:
        return None, session, tracker_profile, tracker_session

    # There is profile

    # Load profile
    # Calling self._get_profile(session) revolves inconsistencies such as - missing ids.
    flat_profile, session, tracker_profile, tracker_session = await _get_profile(tracker_profile, tracker_session,
                                                                                 session, static=static)

    # Check consistency

    if session and tracker_session:
        # Correct session ID are when they are the same or a shadowed session was created when there was a conflict.
        correct_session = tracker_session.id == session.id or session.id == get_shadow_session_id(session.id)
...
) -> Tuple[Optional[FlatProfile], Optional[Session], Optional[PrimaryEntity], Optional[Union[DefaultEntity, Entity]]]:
    # Check if profile should have static ID

    if profile_less is True:
        flat_profile = None
    else:
        flat_profile, session, tracker_profile, tracker_session = await get_profile_and_session(
            session,
