...
#
#         routes = list(await workflow_trigger_dao.load_rule(event_type, source_id))
#
...
#         return profile
#
#     async def _get_routing_rules(self, events: List[Event]) -> Optional[List[Tuple[List[Rule], Event]]]:
#
...
#             event_rules = await _load_by_source_and_events(self.tracker_payload.source.id, events)
#
#         return event_rules
#
#     async def _get_rules_engine(self, profile, session, events: List[Event]) -> Optional[RulesEngine]:
#         # Get routing rules if workflow is not disabled
#
#         event_trigger_rules = await self._get_routing_rules(events)
#
...
#
#         try:
#             rules_engine = await self._get_rules_engine(self.profile, self.session, events)
#
#             #  If no event_rules for delivered event then no need to run rule invoke
#             #  and no need for profile merging
...
#                         self.profile = await self._merge_profile(self.profile)
#
#                 except Exception as e:
