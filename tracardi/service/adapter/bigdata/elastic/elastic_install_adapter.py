        ...
        for key, index in Resource().resources.items():  # type: str, Index
            ...
            if index.multi_index:
                ...
                if get_context().is_production():
                    has_alias = await self.client.alias_exists(_alias)
                else:
                    has_alias = await self.client.alias_exists(_alias, index=_template_pattern)
                ...
                if get_context().is_production():
                    ...
                    has_alias = await self.client.alias_exists(_alias, index=_index)
