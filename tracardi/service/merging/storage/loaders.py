...
async def load_duplicated_profiles_with_ids(profile_ids) -> List[Profile]:
    records = await bd_apm_adapter.load_duplicated_profiles_with_ids(profile_ids)
    profiles = []
...
async def load_duplicated_profiles_with_merge_key(merge_key_values: List[Tuple[str, str]],
    ...
    profiles = await bd_apm_adapter.load_duplicated_profiles_with_merge_key(merge_key_values,
        condition=condition,
        limit=limit)
