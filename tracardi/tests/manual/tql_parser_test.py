...
if __name__ == "__main__":
    ...
    payload = data
    resource = Resource(id="3", type="event", name="name")
    context = {}
    memory = {}
    event = Event(metadata=EventMetadata(time=EventTime()),
                  id="event-id", type="type", source=resource, context=context, profile=profile, session=session, name='type')
    flow = Flow(id="flow-id", name="flow", wf_schema=FlowSchema(version="0.6.0"), type="collect")
